...
function CreateDragContainer(myContainters){
	/*
	...
	*/
	for(var i=0; i<myContainters.length; i++){
	    if(!myContainters[i]){
		   continue;
		}
		var cObj = myContainters[i];
...
function dragInit(){
    ...
    var myContainters=[];
	for(var x=1;x<=7;x++){
	   myContainters[x]=document.getElementById("DragContainer"+x);
	} 
	
	CreateDragContainer(myContainters);
