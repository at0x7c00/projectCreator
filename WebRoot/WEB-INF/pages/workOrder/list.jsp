	...
	<body class="ui-widget-content">
		...
		<table border="0" style="position: absolute; top: 0; right: 0;">
			<tr>
				...
				<td>
					<span><s:a action="workOrder_addUI.action">添加</s:a>
					</span>
				</td>
				<td>
					&nbsp;
				...
				<td>
					<s:a href="workOrder_delete.action"
						onClick="return operation('delete','listForm','workOrder_delete.action','ids')">删除</s:a>
				</td>
			</tr>
		</table>

		<FORM ACTION="${basePath}workOrder_list.action" METHOD="POST"
				...
				<tbody id="listTBody">
					...
					<c:forEach items="${pageBean.recordList}" var="tempRecord">
						<TR id="listTable">
							...
							<td>
								<a href="workOrder_updateUI.action?id=${tempRecord.id}">修改</a>&nbsp;|&nbsp;
								<a href="workOrder_delete.action?ids=${tempRecord.id}"
									onclick="return window.comfirm('确认要删除该条记录吗?');">删除</a>
							</td>
			...
			<div id="InputDetailBar">
				<s:a action="role_addUI.action" cssClass="ButtonStyle"
					onClick="return operation('add','listForm','${basePath}workOrder_addUI.action','');">
				增 加
			</s:a>
				<s:a action="role_delete.action" cssClass="ButtonStyle"
					onClick="return operation('delete','listForm','${basePath}workOrder_delete.action$,'ids')">
				删 除
			</s:a>
			</div>
		</FORM>
