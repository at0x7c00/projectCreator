...
<html>
	...
	<body class="ui-widget-content">
		...
		<form id="formID" METHOD="POST" NAME="fmOperator"
			...
			action="${basePath }rule_update.action">
				...
				<div class="ItemBlock">
					<table align="center" class="mainForm" style="width: 100%;">
						<tr>
							...
							<td width="40%">
								...
								<c:forEach items="${templateList}" var="vari">
									<option value="${vari.id}" ${rule.template.id==vari.id?'selected':''}>
										${vari.name}
									</option>
								</c:forEach>
								</select>
						...
						<tr>
							...
							<td >
								<select name="append">
									<option value="0" ${rule.append ? '':'selected'}>否</option>
									<option value="1" ${rule.append ? 'selected':''}>是</option>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td width="40%">
								<input name="rule.outputFileName" id="rule.outputFileName" type="text"
									size="15" class="InputStyle" value="${rule.outputFileName}"
									vl="validate[required,maxSize[255]]" style="width:60%"/>
								&nbsp;
								<font color="red">*</font>
							</td>
						...
						<tr>
							...
							<td width="40%">
								<input name="rule.cause" id="rule.cause" type="text"
									size="15" class="InputStyle" value="${rule.cause}"
