...
<html>
	...
	<table width="100%">
	  <tr style="border:1px solid red;">
		...
		<form METHOD="POST" NAME="fmOperator" id="formID" style="margin-top: 15; margin-bottom: 0" action="${basePath}package_update.action" >
					...
					<td width="85%" class="TextStyle" align="left" height="16"
						...
						<select name="projectId" class="SelectStyle" id="projectId" vl="validate[required]">
							...
							<c:forEach items="${projectList}" var="project">
								<option value="${project.id}" ${pack.project.id==project.id?'selected':''}>${project.name }</option>
							</c:forEach>
						</select>
						&nbsp;
				...
				<tr>
					...
					<td width="85%" class="TextStyle" align="left" height="16"
						...
						<input type="text" name="pack.name" maxlength="100" class="InputStyle"
							vl="validate[required]" id="title" value="${pack.name }"> <font color="red">*</font>
					</td>
				</tr>
				<tr>
					<td width="15%" class="TextStyle" align="right" height="16"	valign="middle">
					...
					<td width="85%" class="TextStyle" align="left" height="16"
						...
						<select name="parentId" class="SelectStyle">
							...
							<c:forEach items="${parentPacks}" var="parent">
								<option value="${parent.id}" ${pack.parent.id==parent.id?'selected':'' }>${parent.name }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
