	...
	<head>
		...
		<script type="text/javascript">
		   function exportExcle(){
		   		loaded1 =true;
		   		jQuery("#listForm").attr("action","${basePath}node_export.action");
		   		jQuery("#listForm").attr("target","_blank");
		        document.getElementById("listForm").submit();
		   		jQuery("#listForm").attr("action","${basePath}node_list.action");
...
<td class="ui-icon ui-icon-carat-2-n-s"></td><td><span><a href="javascript:void(0);" onclick="jQuery('#causeTab').toggle();">查询条件</a> </span></td> <td>&nbsp;</td>
			...
			<td>
				<span><s:a action="node_addUI.action">添加</s:a></span> 
			</td> 
			<td>&nbsp;</td>
			<td class="ui-icon ui-icon-trash"></td>
			<td>
			 <s:a href="node_delete.action" onClick="return operation('delete','listForm','node_delete.action','ids')">删除</s:a>
			</td>
		...
		<FORM ACTION="${basePath}node_list.action" METHOD="POST" id="listForm"
			NAME="QueryForm">
			<input type="hidden" name="pageNum" value="${pageBean.pageNum }" />
<table width="100%" border="0" id="causeTab" style="display:none;"><tr><td width="10%" align="right">名称：</td><td width="40%"><input name="node.name" id="node.name" type="text" size="15" class="InputStyle"  value="${node.name}" /></td><td width="10%" align="right">处理页面：</td><td width="40%"><select name="pageId" id="pageId" class="SelectStyle"/><option value="">请选择</option><c:forEach items="${pageList}" var="vari"><option value="${vari.id}" ${pageId==vari.id?'selected':''}>${vari.title}</option></c:forEach></select></td></tr><tr><td colspan="4" align="center"><input type="submit" name="button" id="button" value="查 询"class="ButtonStyle"/><input type="reset" name="button2" id="button2" value="重 置" class="ButtonStyle" /></td></tr></table>
...
<td  width="10%"><a href="node_updateUI.action?id=${tempRecord.id}&pageSize=${pageSize}&pageNum=${pageNum}">修改</a>&nbsp;|&nbsp;<a href="node_delete.action?ids=${tempRecord.id}&pageSize=${pageSize}&pageNum=${pageNum}" onclick="return window.confirm('确认要删除该条记录吗?');">删除</a></td>
			...
			<div  id="InputDetailBar">
				<s:a action="node_addUI.action" cssClass="ButtonStyle"
					onClick="return operation('add','listForm','${basePath}node_addUI.action','');">
				增 加
			</s:a>
				<s:a action="node_delete.action" cssClass="ButtonStyle"
					onClick="return operation('delete','listForm','${basePath}node_delete.action','ids')">
				删 除
			</s:a>
		   </div>
		</FORM>
